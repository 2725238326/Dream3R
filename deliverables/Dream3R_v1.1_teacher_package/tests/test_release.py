...
def test_v11_round_trips_both_release_checkpoints(tmp_path) -> None:
    model = build_dream3r_v11_release()
    kitti_checkpoint = tmp_path / "kitti.pt"
    eth3d_checkpoint = tmp_path / "eth3d.pt"

    torch.save(
        {
            "decoder_state_dict": model.kitti_model.decoder.state_dict(),
            "config": vars(model.config.kitti),
        },
        kitti_checkpoint,
    )
    torch.save(
    ...
    loaded = build_dream3r_v11_release(
        kitti_checkpoint=kitti_checkpoint,
        eth3d_checkpoint=eth3d_checkpoint,
    )

    for domain, experts in (("kitti", 3), ("eth3d", 4)):
        pointmaps = torch.randn(1, experts, 2, 5, 3)
        confidences = torch.rand(1, experts, 2, 5, 1)
        memory = torch.randn(1, 32)
        out = loaded(pointmaps, confidences, memory, domain=domain)
        assert out["final_pointmap"].shape == (1, 2, 5, 3)
