"""Dream3R-v0.6 SCF -- train multi-expert fusion head + B0-B4 baselines.
...
Never touches v0.3/v0.5 core. Reuses the differentiable abs_rel loss and the
per-domain split from ``train_fusion_head`` and the eval metric from
``build_oracle_expert_labels``.
"""
