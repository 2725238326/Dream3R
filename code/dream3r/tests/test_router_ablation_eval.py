...
def _write_three_expert_fixtures(root: Path) -> tuple[Path, Path]:
    regime_path = root / "regime_labels.json"
    oracle_path = root / "oracle_labels.json"
    ...
    return regime_path, oracle_path


def test_eval_router_ablation_rejects_feature_mode_mismatch():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        regime_path, oracle_path = _write_three_expert_fixtures(root)
        ckpt_dir = root / "router"
        output = root / "results.json"
        ...
        with pytest.raises(ValueError, match="feature_mode mismatch"):
            evaluate_router_ablation(
                regime_labels=str(regime_path),
                oracle_labels=str(oracle_path),
                router_checkpoint=str(ckpt_dir / "latest.pt"),
                output=str(output),
...
def test_eval_router_ablation_uses_frozen_stats_from_checkpoint():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        regime_path, oracle_path = _write_three_expert_fixtures(root)
        ckpt_dir = root / "router"
        ...
        train_summary = train_router_only(
            regime_labels=str(regime_path),
            oracle_labels=str(oracle_path),
            output_dir=str(ckpt_dir),
            epochs=120,
        ...
        result = evaluate_router_ablation(
            ...
            output=str(output),
            feature_mode="regime_stats",
            sequence_filter=["dense_a", "sparse_a", "dynamic_a"],
        )

        # On a 3-sequence held-out subset, normalization must be the training
        # stats, not the subset stats; the recorded feature_meta should reflect
        # that and match training stat_mean/std exactly.
        assert result["feature_meta"]["stats_frozen"] is True
        assert result["feature_meta"]["stat_mean"] == train_summary["feature_meta"]["stat_mean"]
        assert result["feature_meta"]["stat_std"] == train_summary["feature_meta"]["stat_std"]
